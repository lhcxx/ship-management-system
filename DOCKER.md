...
- Invalid SA password (must meet complexity requirements)
...
**Check:**
1. SQL Server is healthy: `docker compose ps`
2. Connection string is correct in `docker-compose.yml`
3. Network connectivity: `docker compose exec api ping sqlserver`
...
**Cause:** Volume not configured or removed

**Solution:**
```bash
# Check volumes
docker volume ls | grep ship-management

# Use named volume (already configured in docker-compose.yml)
...
3. Optimize Dockerfile (multi-stage builds already implemented)
